...
public:
                                    ...
                                    : rootPath(rootPath), filename(fn), isDownSample(isDownSample)
    {
        ...
        else if (rootPath.find("argo") != std::string::npos)
        {
            type = DataType::BINARIES_ARGO; 
        }
        else
        {
...
private:
    ...
    std::vector<std::vector<float>> readFromBinaryArgo(std::string &filename)
    {
        std::vector<double> centers;
...
public:
    ...
    bool isRun()
    {
        if (type == DataType::BINARIES || type == DataType::BINARIES_KITTI || type == DataType::BINARIES_ARGO)
        {
    ...
    void operator >>(std::vector<std::vector<float>> &pointcloud) 
    {
        ...
        else if (type == DataType::BINARIES_ARGO)
        {
            pointcloud = readFromBinaryArgo(binaryFiles[currentFrame++]);
        }
        else if (type == DataType::PCAP)
        {
