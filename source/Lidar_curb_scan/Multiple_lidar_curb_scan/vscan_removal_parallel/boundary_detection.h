...
public:
    ...
    Boundary_detection(int numOfScan, float sensor_height, std::string data_path, float theta, cv::Mat rot, cv::Mat trans): 
                        dataReader(data_path, theta, rot, trans), 
                        tilted_angle(tilted_angle), sensor_height(sensor_height) 
    {
        num_of_scan = numOfScan;
        ranges = std::vector<std::vector<int>>(num_of_scan*2, std::vector<int>(2));
    ...
    bool isRun();
    void retrieveData();
    std::vector<int> retrieveData(std::string);
    void pointcloud_preprocessing(const cv::Mat &rot);
    void pointcloud_preprocessing_pcap();
//     std::vector<std::vector<cv::Vec3f>> runDetection(const cv::Mat &rot, const cv::Mat &trans);
    void runDetection();
    std::vector<std::vector<cv::Vec3f>> getBoundaryBuffers(const cv::Mat &rot, const cv::Mat &trans, const std::vector<std::vector<float>> &vscanRes, std::unordered_map<int, int> &map, std::string &kittiFolder);
    std::vector<std::vector<cv::Vec3f>> getBoundaryBuffersSingle( const std::vector<std::vector<float>> &vscanRes, std::unordered_map<int, int> &map, std::string &kittiFolder);
    ...
    std::vector<cv::viz::WPolyLine> getThirdOrderLines(std::vector<cv::Vec3f> &buf, std::vector<cv::Vec3f>& ultrasonicPoints);
    ...
    std::vector<cv::viz::WPolyLine> getLineFromCoeffs(std::vector<cv::Vec3f> *buf, std::vector<cv::Vec3f> *ultrasonicPoints, std::vector<float>*solution, int option); 
    std::vector<cv::viz::WPolyLine> getLineFromCoeffs(std::vector<cv::Vec3f>&buf, std::vector<float>& solution, int option); 
    std::vector<cv::viz::WPolyLine> getLineFromCoeffs(std::vector<cv::Vec3f>&buf, std::vector<cv::Vec3f>&ultrasonicPoints, std::vector<float>&solution, int option); 
    ...
    void writeResultTotxt(const std::vector<float> &boundaryCoeffs, int leftRight, std::string &kittiFolder);
    void writeResultTotxttracking(const std::vector<float> &boundaryCoeffs, int leftRight, std::string &kittiFolder);
    void rotate_and_translate_multi_lidar_yaw(const cv::Mat &rot);
    void rotate_and_translate_multi_lidar_yaw_return(const cv::Mat &rot, std::vector<cv::Vec3f>&pntcld);
    // void put_pointcloud(std::vector<std::vector<float>>&points);
    // void put_pointcloud(std::vector<cv::Vec3f> points);
    void put_pointcloud_floats(std::vector<float> points);
    void put_pointcloud(std::vector<std::vector<float>> &pointcloud_ );
...
private:
    ...
    // Camera
    // std::vector<std::vector<float>> lidar_to_image_ours = {{6.07818353e+02, -7.79647962e+02, -8.75258198e+00, 2.24308511e+01},
