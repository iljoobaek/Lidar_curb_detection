...
#define USE_MULTIPLE_LIDAR true
...
public:
    Boundary_detection(string dir, int id, float tilted_angle, float sensor_height): directory(dir), frame_id(id), num_of_scan(16) {
        ...
        this->ranges = std::vector<std::vector<int>>(16, std::vector<int>(2));
        #else
        ...
        if (dir.find(".pcap") != string::npos) {
            this->isPCAP = true;
            this->capture = std::unique_ptr<velodyne::VLP16Capture> (new velodyne::VLP16Capture(dir));
        }
        else {
            this->isPCAP = false;
        }
