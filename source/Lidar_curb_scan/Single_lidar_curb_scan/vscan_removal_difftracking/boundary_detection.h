...
//     std::vector<std::vector<cv::Vec3f>> runDetection(const cv::Mat &rot, const cv::Mat &trans);
    std::vector<std::vector<cv::Vec3f>> runDetection(const cv::Mat &rot, const cv::Mat &trans, const std::vector<std::vector<float>> &vscanRes, std::unordered_map<int, int> &map, std::string &kittiFolder);
