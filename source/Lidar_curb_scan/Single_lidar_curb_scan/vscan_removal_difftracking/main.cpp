...
//                 std::cout<<"out val "<<PyFloat_AsDouble(value)<<"\n";
				data.push_back( PyFloat_AsDouble(value) );
			}
		} else {
...
std::vector<float> TupleToVector_Float(PyObject* incoming) {
	std::vector<float> data;
	if (PyTuple_Check(incoming)) {
		for(Py_ssize_t i = 0; i < PyTuple_Size(incoming); i++) {
			PyObject *value = PyTuple_GetItem(incoming, i);
...
//         }
        thirdOrder = detection.getThirdOrderLines(buf[1]); // Vizualizing [left, right]
		results[0] = detection.get_result_bool(); // * check this
...
//                     std::vector<cv::viz::WPolyLine> resLine = detection.getLineFromCoeffs(buf[1], cvUltrasonicPoints,prev_lcoeff, 0);
                ...
                if(leftPointCount > 50){
                    ...
                    PyObject* prevlList = vectorToList_Float(prev_lcoeff);
                    PyObject* rList = NullListObj();
                    PyObject* prevrList = NullListObj();
                    ...
                    PyTuple_SetItem(pArgs, 4, prevlList);
                    PyTuple_SetItem(pArgs, 5, prevrList);
...
//                             std::vector<cv::viz::WPolyLine> resLine = detection.getLineFromCoeffs(buf[1], cvUltrasonicPoints, prev_lcoeff, 0);
                    ...
                    std::cout<<" contents of lprev "<<prev_rcoeff[0]<<" and "<<prev_lcoeff[1];
...
//                         std::vector<cv::viz::WPolyLine> resLiner = detection.getLineFromCoeffs(buf[1], cvUltrasonicPoints, prev_lcoeff, 0);
                        if(detection.getProblem()){
                             std::cout<<"\nPROBLEM 4 \n";
                             thirdOrder.push_back(prev_lline);
                        }else{
...
//                     std::vector<cv::viz::WPolyLine> resLine = detection.getLineFromCoeffs(buf[1], cvUltrasonicPoints, prev_rcoeff, 1);
...
//                     thirdOrder.push_back(prev_rline);
                ...
                if(rightPointCount > 20){
                    ...
                    PyObject* prevlList = NullListObj();
                    PyObject* rList = vectorToList_Float(rcoeff);
                    PyObject* prevrList = NullListObj();
                    if(!prev_rcoeff.empty()){
                        
                        PyObject* prevrList = vectorToList_Float(prev_rcoeff);
                    }
//                     std::cout<<"Frame idx two : \t"<<frameStart+frame_idx<<"\n";
                    pValue = PyFloat_FromDouble(updateNum);
...
//                             std::vector<cv::viz::WPolyLine> resLiner = detection.getLineFromCoeffs(buf[1], cvUltrasonicPoints, prev_avail_rcoeff, 1);
                            if(detection.getProblem()){
                                std::cout<<"\nPROBLEM 3 \n";
                                thirdOrder.push_back(prev_rline);
                            }else{
...
//                         std::vector<cv::viz::WPolyLine> resLiner = detection.getLineFromCoeffs(buf[1], cvUltrasonicPoints, rcoeff, 1);
                        if(detection.getProblem()){
                             std::cout<<"\nPROBLEM 4 \n";
                             thirdOrder.push_back(prev_rline);
                        }else{
...
//             pValue = PyFloat_FromDouble(frame_idx);
            ...
            try {
                if(PyCallable_Check(pFunc)){
                    ...
                    std::cout<<"Got points rorig "<<rcoeff[0]<<"\t"<<rcoeff[1]<<"\t"<<rcoeff[2]<<"\n";
                    std::cout<<" HREE!!!! \n";
                    //std::cout<<"Got points "<<res_curve[0]<<"\n";
                    //std::cout<<"Got points "<<res_curve[0]<<"\t"<<res_curve[1]<<"\t"<<res_curve[2]<<"\n";
...
// 		std::vector<cv::viz::WPolyLine> gtLines = thirdOrder;
		...
		LidarViewer::updateViewerFromBuffers(buffers, results, viewer, res.first, thirdOrder,gtLine,  temp[0], temp[1],cvUltrasonicPoints);
