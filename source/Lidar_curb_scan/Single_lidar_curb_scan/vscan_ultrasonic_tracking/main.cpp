        ...
        if ( lcoeff.empty() || rcoeff.empty() ) {
            ...
            try {
                if(PyCallable_Check(pFunc)){
                    ...
                    std::vector<cv::viz::WPolyLine> resLinel = detection.getLineFromCoeffs(buf[1], l_curv, 0);
                    std::vector<cv::viz::WPolyLine> resLiner = detection.getLineFromCoeffs(buf[1], r_curv, 1);
