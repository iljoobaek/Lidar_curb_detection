...
def ssd_bboxes_decode(feat_localizations,
    ...
    cy = feat_localizations[:, :, 0] * wref * prior_scaling[0] + yref
    cx = feat_localizations[:, :, 1] * href * prior_scaling[1] + xref
    h = wref * np.exp(feat_localizations[:, :, 2] * prior_scaling[2])
    w = href * np.exp(feat_localizations[:, :, 3] * prior_scaling[3])
...
def ssd_bboxes_select_layer(predictions_layer,
    ...
    if select_threshold is None:
        ...
        sub_predictions = predictions_layer[:, :, label_offset:]
        idxes = np.where(sub_predictions > select_threshold)
        classes = idxes[-1]+label_offset
