...
int main(int argc, char* argv[]) 
{
    ...
    std::string mem_name;
    std::vector<std::vector<float>> Vscan_results;
    ...
    while(!viewer.wasStopped())
    {
        ...
        boundary_display_buffer.resize(2);
        thirdOrder_lines.resize(2);
        pointcloud_buffer.resize(2);
        pointcloud_bndr_buffer.resize(2);
        
        int i = 0;
        int j = 0;
        int vscan_size =0;
        for(int k=0; k<numOfLidars; k++){
            ...
            mem_name = "Lidar_update_status_"+std::to_string(k);
            bi::managed_shared_memory segment_status(bi::open_or_create ,mem_name.c_str(),1000);
            std::cout << "Finding status:\n";
            int *updateStatus = segment_status.find<int>("Update_status").first;
            // if(*updateStatus != 1){
            //     std::cout<<"CONTINUING ... \n";
            //     continue;
            // }
            ...
            if(res.first){
                ...
                Vscan_results.resize(vscan_size);
                for(auto it = bVec_vscan->begin(); it!=bVec_vscan->end(); it=it+4){
                    Vscan_results[j].resize(4);
                    Vscan_results[j][0] = (float)*it;
                    Vscan_results[j][1] = (float)*(it+1);
                    Vscan_results[j][2] = (float)*(it+2);
                    Vscan_results[j][3] = (float)*(it+3);
                    j++;
                }
                std::cout << "Pushed successfully \n";
                ...
                std::cout<<"The result of" + mem_name + "is empty \n";
                Vscan_results.resize(1);
                Vscan_results[0].resize(4);
                Vscan_results[0][0] = 0;
                Vscan_results[0][1] = 0;
                Vscan_results[0][2] = 0;
                Vscan_results[0][3] = 0;
