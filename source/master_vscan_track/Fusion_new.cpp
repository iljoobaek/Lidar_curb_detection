...
//     cv::Vec3f firstPoint(float,float,float);
    ...
    std::vector<std::vector<cv::Vec3f>> bothLR;
...
//                  newOriginR = points[i];
//                  std::cout<<"--------------- R Prior selection: x "<<newOriginR[0]<<" y "<<newOriginR[1]<<"\n";
//              }
//              if (points[i][0] < 0 && newOriginL == cv::Vec3f(0, 0, 0)) {
//                  newOriginL = points[i];
//                  std::cout<<"--------------- L Prior selection: x "<<newOriginL[0]<<" y "<<newOriginL[1]<<"\n";
//              }
//          }
        
// //          for (auto i: sort_indexes(Rdistances)) {
// //               newOriginR = points[i] ;
// //               previousRPoint = newOriginR;
// //               break;
// //          }
// //          for (auto i: sort_indexes(Ldistances)) {
// //               newOriginL = points[i] ;
// //               previousLPoint = newOriginL;
// //               break;
// //          }
        
// //         std::cout<<"--------------- R Prior selection: x "<<newOriginR[0]<<" y "<<newOriginR[1]<<" d "<<Rdistances[0]<<"\n";
// //         std::cout<<"--------------- L Prior selection: x "<<newOriginL[0]<<" y "<<newOriginL[1]<<" d "<<Ldistances[0]<<"\n";
        
        
//  /////////////////////////////////////////////////////////////////////////////////////////       
//         cv::Vec3f RcenterInit;
//         if(prevRCenterInit == cv::Vec3f(0, 0, 0))
//         {
//             //// Initial: Frame 1:
//             // Select 10 points 
//             std::vector<float> tenRPoints;
//             for (int i = 0; i < 10; i++) {
//                 tenRPoints.push_back(Rpoints[i][0]);
//             }

//             // Calculate x mean
//             float x_sum = 0.0;
//             for (int i = 0; i < tenRPoints.size(); i++) {
//                     x_sum = x_sum + tenRPoints[i];
//             }
//             float x_mean = x_sum/(tenRPoints.size()+0.001);

//             // Find closest to x_mean
//             std::vector<float> tenRDists;
//             for (int i = 0; i < tenRPoints.size(); i++) {
//                     tenRDists.push_back(std::abs(tenRPoints[i] - x_mean));
//             }
            
//             for (auto i: sort_indexes(tenRDists)) {
//                   RcenterInit = Rpoints[i] ; // First center
//                   std::cout<<"First center found \n";
//                   break;
//              }
//              prevRCenterInit = RcenterInit;
//              // 
//         }
//         else
//         {
//             //// Frame 2: Find point within minium distance from center
//             std::vector<float> Rdistances;
//              for(int i = 0; i < 20; i++) {
//                  Rdistances.push_back(distance(prevRCenterInit[0], prevRCenterInit[1], Rpoints[i][0], Rpoints[i][1])) ;
//             }
//             for (auto i: sort_indexes(Rdistances)) {
//                   RcenterInit = Rpoints[i] ; // Second center
//                   std::cout<<"Second center found \n";
//                   break;
//              }
         
//              // Attenuate Jumps
//              if(std::abs(RcenterInit[0] - prevRCenterInit[0]) < thresCenter){ // Choose second center
//                  newOriginR = RcenterInit;
//                  prevRCenterInit = RcenterInit;
//              }
//              else{
//                  newOriginR = prevRCenterInit; // Choose 1st center
//              }
//                 std::cout<<"--------------- R New selection: x "<<newOriginR[0]<<" y "<<newOriginR[1]<<"\n";
            
//          }
// ///////////////////////////////////////////////////////////////////////////////
        
// /////////////////////////////////////////////////////////////////////////////////////////       
//         cv::Vec3f LcenterInit;
//         if(prevLCenterInit == cv::Vec3f(0, 0, 0))
//         {
//             //// Initial: Frame 1:
//             // Select 10 points 
//             std::vector<float> tenLPoints;
//             for (int i = 0; i < 10; i++) {
//                 tenLPoints.push_back(Lpoints[i][0]);
//             }

//             // Calculate x mean
//             float x_sum = 0.0;
//             for (int i = 0; i < tenLPoints.size(); i++) {
//                     x_sum = x_sum + tenLPoints[i];
//             }
//             float x_mean = x_sum/(tenLPoints.size()+0.001);

//             // Find closest to x_mean
//             std::vector<float> tenLDists;
//             for (int i = 0; i < tenLPoints.size(); i++) {
//                     tenLDists.push_back(std::abs(tenLPoints[i] - x_mean));
//             }
            
//             for (auto i: sort_indexes(tenLDists)) {
//                   LcenterInit = Lpoints[i] ; // First center
//                   std::cout<<"First center found \n";
//                   break;
//              }
//              prevLCenterInit = LcenterInit;
//              // 
//         }
//         else
//         {
...
//              for(int i = 0; i < 20; i++) {
//                  Ldistances.push_back(distance(prevLCenterInit[0], prevLCenterInit[1], Lpoints[i][0], Lpoints[i][1])) ;
//             }
//             for (auto i: sort_indexes(Ldistances)) {
//                   LcenterInit = Lpoints[i] ; // Second center
//                   std::cout<<"Second center found \n";
//                   break;
//              }
         
//              // Attenuate Jumps
//              if(std::abs(LcenterInit[0] - prevLCenterInit[0]) < thresCenter){ // Choose second center
//                  newOriginL = LcenterInit;
//                  prevLCenterInit = LcenterInit;
//              }
//              else{
//                  newOriginL = prevLCenterInit; // Choose 1st center
//              }
//                 std::cout<<"--------------- L New selection: x "<<newOriginL[0]<<" y "<<newOriginL[1]<<"\n";
            
//          }
// ///////////////////////////////////////////////////////////////////////////////
        
                                       
// //         std::vector<float> LcenterInit;
// //         if(prevLCenterInit == cv::Vec3f(0, 0, 0))
// //         {
// //             //// Initial: Frame 1:
// //             // Select 10 points 
// //             std::vector<float> tenRPoints;
// //             for (int i = 0; i < 10; i++) {
// //                 tenPoints.push_back(Lpoints[i][0]);
// //             }

// //             // Calculate x mean
// //             float x_sum = 0.0;
// //             for (int i = 0; i < tenPoints.size(); i++) {
// //                     x_sum = x_sum + tenPoints[i][0];
// //             }
// //             x_mean = x_sum/(tenPoints.size()+0.001)

// //             // Find closest to x_mean
// //             std::vector<float> tenDists;
// //             for (int i = 0; i < tenPoints.size(); i++) {
// //                     tenDists.push_back(std::abs(tenPoints[i] - x_mean);
// //             }
            
// //             for (auto i: sort_indexes(tenDists)) {
// //                   LcenterInit = Rpoints[i] ; // First center
// //                   break;
// //              }
// //              prevRCenterInit = LcenterInit;
// //              // 
// //         }
// //         else
// //         {
...
// //             }
// //             for (auto i: sort_indexes(Rdistances)) {
// //                   LcenterInit = Rpoints[i] ; // Second center
// //                   break;
// //              }
         
// //              // Attenuate Jumps
// //              if(std::abs(LcenterInit - prevRCenterInit) < thresCenter){
// //                  newOriginR = LcenterInit;
// //                  prevRCenterInit = LcenterInit;
// //              }
// //              else{
// //                  newOriginR = prevRCenterInit;
// //              }
...
//          std::vector<std::vector<cv::Vec3f>> bothLR = {tempL, tempR};
//          return bothLR;
        ...
        else
        {
            ...
            for (auto i: sort_indexes(Rdistances)) {
             ...
             if(std::abs(RcenterInit[0] - prevRCenterInit[0]) < thresCenter){ // Choose second center
                ...
                std::cout<<"--------------- 2R New selection: x "<<newOriginR[0]<<" y "<<newOriginR[1]<<"\n";
        ...
        else
        {
            ...
            for (auto i: sort_indexes(Ldistances)) {
             ...
             if(std::abs(LcenterInit[0] - prevLCenterInit[0]) < thresCenter){ // Choose second center
                ...
                std::cout<<"--------------- 2L New selection: x "<<newOriginL[0]<<" y "<<newOriginL[1]<<"\n";
