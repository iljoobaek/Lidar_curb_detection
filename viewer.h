...
void updateViewerFromBuffers(std::vector<std::vector<cv::Vec3f>> &buffers, std::vector<std::vector<bool>> &results, cv::viz::Viz3d &viewer, std::vector<std::vector<float>> &vscanRes, std::vector<cv::viz::WPolyLine> &polyLine, cv::viz::WPolyLine &gtLine, std::vector<cv::Vec3f> transformedL, std::vector<cv::Vec3f> transformedR) 
{
    ...
    for (int i = 0; i < transformedL.size(); i++){
        ...
        if (i == buffers.size()-1) 
        {
            collection.addCloud(transformedLMat, cv::viz::Color::purple()); //L - purple
    ...
    viewer.showWidget("Poly gt", gtLine);
